...
export function renderFindings(c, entries, r, ref, t = today()) {
  ...
  ${st.p2 ? `<tr><td>Phase 2 — reconciliation-flagged</td><td class="n">${money(st.p2)}</td><td>Reconciliation flag on the entry; same ${r.p1}-day window as Phase 1</td><td>Partner broker files in CAPE Phase 2</td></tr>` : ""}${st.manual ? `<tr><td>Manual path</td><td class="n">${money(st.manual)}</td><td>Surety-paid, drawback or AD/CVD-suspended entries; not eligible for Phase 1</td><td>Confirm the path with the broker; not counted as recoverable now</td></tr>` : ""}${st.future ? `<tr><td>Future phase — reconciliation on file</td><td class="n">${money(st.future)}</td><td>Reconciliation entry already on file; CBP has not opened this phase</td><td>Tracked; no action yet</td></tr>` : ""}
