...
export function clientStats(c, entries, r, t = today()) {
  ...
  const p1 = sum(x => x.p.phase.startsWith("Phase 1")), pr = sum(x => x.p.phase.startsWith("Protest")), gone = sum(x => x.p.phase.startsWith("Phase 3")), na = es.filter(x => x.p.na).length, est = es.filter(x => x.p.est && !x.p.na).length;
