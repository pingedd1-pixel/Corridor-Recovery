...
export function dashboard(clients, entries, r, t = today()) {
  ...
  entries.forEach(e => { const p = phase(e, r, t); if (p.na) { na++; return; } duty += p.duty; if (p.phase.startsWith("Phase 1")) p1 += p.duty; else if (p.phase.startsWith("Protest")) pr += p.duty; else if (p.phase.startsWith("Phase 3")) gone += p.duty; if (p.days != null) { if (p.days < 0) past++; else if (p.days < r.urg) urg++; } if (p.est) est++; });
